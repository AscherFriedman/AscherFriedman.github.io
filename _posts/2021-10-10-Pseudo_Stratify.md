...
```python
def test_various_splits(df,split_vals,horizontal_line=None):
    ...
    if horizontal_line!=None:
        plt.axvline(horizontal_line,ls='--',color='grey')
        plt.text(horizontal_line+1,np.max(test_mean_devs)*.75,'Recommended Split',rotation=0)
```
...
test_various_splits(df_samples,np.arange(0,50,1),horizontal_line=recommended_splits)
```
...
```python
round(((np.min([n_samples,10000]))*.2)**.5)
...
print('Recommended Split:',recommended_splits)
test_various_splits(df_samples,np.arange(0,recommended_splits*2,2),horizontal_line=recommended_splits)
```
